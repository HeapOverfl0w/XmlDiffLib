...
{
    public delegate bool CompareFunction(string x, string y);
    ...
    public class XmlDiff
    {
        ...
        private bool CompareText(XPathNavigator xmlFromNav, XPathNavigator xmlToNav, out XmlDiffNode result, ref int diffNumber)
        {
            ...
            {
                if ((options.CompareFunction != null && !options.CompareFunction(xFrom.Value, xTo.Value) && !CompareTextValue(xFrom.Value, xTo.Value)) ||
                    (options.CompareFunction == null && !CompareTextValue(xFrom.Value, xTo.Value)))
                {
                    result = new XmlDiffNode
                    {
                        XPath = GetXPath(xFrom),
